...
class CrossviewModel(nn.Module):
    ...
    def _forward_dinov3_360(self, ground_input, aerial_input, debug=False):

        # Process Aerial Image (Standard)
        aerial_outputs = self.model(aerial_input)
        ...
        # Process Ground Image (Split into 4 tiles)
        ground_batch_size, _, ground_img_height, ground_img_width = ground_input.shape
        
        # Assuming 4 tiles for 360 view
        num_tiles = 4
        tile_width = ground_img_width // num_tiles
        
        tiles = []
        for i in range(num_tiles):
            tiles.append(ground_input[:, :, :, i*tile_width : (i+1)*tile_width])
            
        # Stack tiles into batch dimension: (B*4, C, H, W/4)
        stacked_tiles = torch.cat(tiles, dim=0)
        
        ground_outputs = self.model(stacked_tiles)
        ground_last_hidden_states = ground_outputs.last_hidden_state
        
        # Extract patches
        ground_num_patches_height = ground_img_height // self.patch_size
        ground_num_patches_width_tile = tile_width // self.patch_size
        
        ground_patch_features_flat_tile = ground_last_hidden_states[:, 1 + self.model.config.num_register_tokens:, :]
        
        # Reshape to spatial dimensions: (B*4, H_p, W_p_tile, D)
        ground_patch_features_tile = ground_patch_features_flat_tile.unflatten(1, (ground_num_patches_height, ground_num_patches_width_tile))
        
        # Reshape to separate tiles: (num_tiles, B, H_p, W_p_tile, D)
        ground_patch_features_tile = ground_patch_features_tile.view(num_tiles, ground_batch_size, ground_num_patches_height, ground_num_patches_width_tile, -1)
        
        # Permute to (B, num_tiles, H_p, W_p_tile, D)
        ground_patch_features_tile = ground_patch_features_tile.permute(1, 0, 2, 3, 4)
        
        # Concatenate tiles along width (dim 3) -> (B, H_p, W_p_total, D)
        ground_patch_features = torch.cat([ground_patch_features_tile[:, i] for i in range(num_tiles)], dim=2)
        
        # Flatten spatial dimensions: (B, H_p * W_p_total, D)
        ground_patch_features_flat = ground_patch_features.flatten(1, 2)
        
        return ground_patch_features_flat, aerial_patch_features_flat

    def _forward_dinov3_180(self, ground_input, aerial_input, debug=False):
        ...
        # Process Ground Image (Split into 2 tiles)
        ground_batch_size, _, ground_img_height, ground_img_width = ground_input.shape
        
        # Assuming 2 tiles for 180 view
        num_tiles = 2
        tile_width = ground_img_width // num_tiles
        ...
        # Stack tiles into batch dimension: (B*2, C, H, W/2)
        stacked_tiles = torch.cat(tiles, dim=0)
        ...
        # Reshape to spatial dimensions: (B*2, H_p, W_p_tile, D)
        ground_patch_features_tile = ground_patch_features_flat_tile.unflatten(1, (ground_num_patches_height, ground_num_patches_width_tile))
    ...
    def forward(self, ground_input, aerial_input, debug=False):
        ...
        if self.backbone == 'clip':
            ...
            if self.fov == 360 or (isinstance(self.fov, tuple) and self.fov[0] == 360):
                return self._forward_dinov3_360(ground_input, aerial_input, debug)
            elif self.fov == 180 or (isinstance(self.fov, tuple) and self.fov[0] == 180):
                return self._forward_dinov3_180(ground_input, aerial_input, debug)
    ...
    def get_combined_embedding_visualization(self, tokens1, tokens2, grid_size1, grid_size2, sky_grid=None, random_state=20, debug=False):
        ...
        if sky_grid is not None:
            if sky_grid.ndim == 2:
                sky_grid = sky_grid.flatten()
            
            # Assume sky_grid > 0 is sky
            is_sky = sky_grid > 0
            
            if is_sky.shape[0] == token1_shape:
                tokens1_ground = tokens1[~is_sky]
                tokens_for_pca = np.concatenate((tokens1_ground, tokens2), axis=0)
                pca.fit(tokens_for_pca.astype(np.float32))
                reduced_tokens = pca.transform(combined_tokens.astype(np.float32))
            else:
                if debug:
                    print(f"Sky grid shape {sky_grid.shape} mismatch with tokens1 {token1_shape}")
...
class Dinov2Matcher:
    ...
    def get_combined_embedding_visualization(self, tokens1, tokens2, grid_size1, grid_size2, sky_grid=None, random_state=20):
        pca = PCA(n_components=3, random_state=random_state)
